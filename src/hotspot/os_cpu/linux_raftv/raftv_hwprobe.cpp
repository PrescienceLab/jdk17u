...
#ifndef NR_raftv_hwprobe
#ifndef NR_arch_specific_syscall
#define NR_arch_specific_syscall 244
#endif
#define NR_raftv_hwprobe (NR_arch_specific_syscall + 14)
#endif

struct raftv_hwprobe {
    int64_t key;
    uint64_t value;
};

long sys_raftv_hwprobe(struct raftv_hwprobe *pairs, size_t pairc,
                       size_t cpuc, cpu_set_t *cpus,
                       unsigned int flags) {
  return syscall(NR_raftv_hwprobe, pairs, pairc, cpuc, cpus, 0 /* flags*/);
}

static bool rw_hwprobe_completed = false;

static struct raftv_hwprobe query[] = {{RISCV_HWPROBE_KEY_MVENDORID, 0},
...
bool RiscvHwprobe::probe_features() {
  ...
  int ret = sys_raftv_hwprobe(&query[0], sizeof(query) / sizeof(query[0]), 0, nullptr, 0);
  rw_hwprobe_completed = true;
  if (ret != 0) {
    log_debug(os, cpu)("raftv_hwprobe unsupported");
    return false;
  }
  log_debug(os, cpu)("raftv_hwprobe supported");
