...
enum LIR_Code {
  ...
  , begin_opAssert
    , lir_assert
  , end_opAssert
#if (defined(RISCV) || defined(RAFTV)) && defined(INCLUDE_ZGC)
...
class LIR_Op2: public LIR_Op {
 ...
 public:
  LIR_Op2(LIR_Code code, LIR_Condition condition, LIR_Opr opr1, LIR_Opr opr2, CodeEmitInfo* info = NULL, BasicType type = T_ILLEGAL)
    ...
    assert(code == lir_cmp || code == lir_assert RISCV_ONLY(|| code == lir_branch || code == lir_cond_float_branch) 
		                                 RAFTV_ONLY(|| code == lir_branch || code == lir_cond_float_branch), "code check");
  }
  ...
  LIR_Op2(LIR_Code code, LIR_Opr opr1, LIR_Opr opr2, LIR_Opr result = LIR_OprFact::illegalOpr,
    ...
    assert(code != lir_cmp && RISCV_ONLY(code != lir_branch && code != lir_cond_float_branch &&)
		              RAFTV_ONLY(code != lir_branch && code != lir_cond_float_branch &&) is_in_range(code, begin_op2, end_op2), "code check");
  }
